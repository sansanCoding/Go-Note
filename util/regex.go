...
//检测用户名,不能以数字开头
//@return 匹配正确,返回true;匹配不对,返回false
func (thisObj *regex) CheckUserName(userName string) bool {
	reg := `^[a-zA-Z\p{Han}]+[a-z0-9A-Z\p{Han}]+$`
	ok, err := regexp.MatchString(reg, userName)
	if err != nil || !ok {
		return false
	}
	return true
...
//检测手机号码格式是否正确
func (thisObj *regex) CheckMobile(phone string) bool {
...
//检测邮箱格式是否正确
func (thisObj *regex) CheckEmail(email string) bool {
	regular := `\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*`
	reg := regexp.MustCompile(regular)
...
//检测是否中文
func (thisObj *regex) CheckIsChineseChar(str string) bool {
	for _, r := range str {
...
//检测QQ号码是否正确
func (thisObj *regex) CheckQQ(qq string) bool {
	regular := `^[1-9]*[1-9][0-9]*$`
	reg := regexp.MustCompile(regular)
	return reg.MatchString(qq)
}

//------------------------------ 基础知识 - Golang 中的正则表达式 start ------------------------------
/*
------------------------------------------------------------
//如下内容转自文章:
