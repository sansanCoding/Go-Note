...
//TCP服务端-开启监听
func (thisObj *TCPServer) serverListen(){
	...
	for {
		...
		if connErr!=nil {
			go thisObj.errChannelAdd(localMsgPrefix+"listenAcceptErr:"+connErr.Error())
			continue
		}
		//调用每一个链接处理
		go thisObj.serverAccept(conn)
	}
}

//TCP服务端-处理每一个链接
func (thisObj *TCPServer) serverAccept(conn net.Conn){
	...
	for {
		...
		//错误检测
		if thisObj.serverAcceptReadErrorCheck(err) {
			break
		}
...
func (thisObj *TCPServer) serverAcceptReadErrorCheck(err error) bool {
	localMsgPrefix := tcpServerMsgPrefix+"serverAcceptReadErrorCheck-"

	//若是读到结尾,则代表没有了
	if err==io.EOF {
		go thisObj.errChannelAdd(localMsgPrefix+"readConnReadEOF!")
		return true
	//若是其他错误
	}else if err!=nil {
		go thisObj.errChannelAdd(localMsgPrefix+"readConnReadErr[ErrType:"+fmt.Sprint(reflect.TypeOf(err))+"]:"+err.Error())
		return true
	}
...
//TCP服务端-错误通道监听
func (thisObj *TCPServer) errChannelListen(){
...
//TCP服务端-添加错误信息到错误通道
func (thisObj *TCPServer) errChannelAdd(errInfo interface{}){
